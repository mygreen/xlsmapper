...
/**
 * {@link CellConverterFactory}のベースとなるクラス。
 *
 * @since 2.0
 * @author T.TSUCHIE
 *
 */
public abstract class AbstractCellConverterFactory<T> implements CellConverterFactory<T> {
     ...
     * @param config
     */
    protected void setup(final AbstractCellConverter<T> cellConverter, final FieldAccessor field, final Configuration config) {
        ...
        // 各個別の設定
        setupType(cellConverter, field, config);
     ...
     * @param config
     */
    protected abstract void setupType(AbstractCellConverter<T> cellConverter, FieldAccessor field, Configuration config);
