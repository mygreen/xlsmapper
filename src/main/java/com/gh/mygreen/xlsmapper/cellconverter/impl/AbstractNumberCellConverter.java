...
public abstract class AbstractNumberCellConverter<T extends Number> extends AbstractCellConverter<T> {
    ...
    protected void setupCell(final Cell cell, final Optional<T> cellValue) throws TypeBindException {
        ...
        if(excelPattern.isPresent() && !templateFormatPatten.equalsIgnoreCase(excelPattern.get())) {
            
            CellStyle style = cell.getSheet().getWorkbook().createCellStyle();
            style.cloneStyleFrom(cell.getCellStyle());
            style.setDataFormat(POIUtils.getDataFormatIndex(cell.getSheet(), excelPattern.get()));
